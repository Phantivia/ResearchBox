...
describe("recommendation markers", () => {
  it("builds include and ignore markers", () => {
    expect(buildIncludeMarker("2401.12345").content[0]).toEqual({
      type: "text",
      text: "【已纳入推荐】2401.12345",
    });
    expect(buildIgnoreMarker("2401.12345").content[0]).toEqual({
      type: "text",
      text: "【已忽略推荐】2401.12345",
    });
  });

  it("parses and detects recommendation markers", () => {
    const include = buildIncludeMarker("2401.12345");
    expect(isRecommendationMarker(include)).toBe(true);
    expect(parseRecommendationMarker("【已纳入推荐】2401.12345")).toEqual({
      decision: "included",
      arxivId: "2401.12345",
    });
  ...
  it("only treats trailing markers as editable after the last real user message", () => {
    const messages = [
      ...
      buildIncludeMarker("2401.1"),
      buildIgnoreMarker("2401.2"),
    ];
    ...
    messages.push(buildIncludeMarker("2401.3"));
  ...
  it("removes editable markers for a specific arxiv id", () => {
    ...
    expect(parseRecommendationMarker("【已忽略推荐】2401.2")).toBeTruthy();
  });

  it("builds and strips composer prefix from decisions", () => {
    const prefix = buildComposerPrefix({
      "2401.1": "included",
      "2401.2": "ignored",
    });
    expect(prefix).toBe("【论文选择备忘】已纳入：2401.1；已忽略：2401.2。 ");
